...
import { readHeartbeatTail, readSessionReceiptsTail, writeMemorableConsent } from '../src/core/context/hook-heartbeat.ts';
...
describe('claude-code lane (session-end hook)', () => {
  ...
  test('an unallowlisted tool shape creates no receipt and never spawns the relay', async () => {
    await optInFully();
    const { marker } = stubRelay();
    const transcriptRoot = join(tmp, 'projects');
    mkdirSync(transcriptRoot, { recursive: true });
    const transcript = join(transcriptRoot, 'sess-unknown-tool.jsonl');
    writeFileSync(transcript, [
      JSON.stringify({ type: 'user', message: { role: 'user', content: 'handle private data' } }),
      JSON.stringify({
        type: 'assistant',
        message: { role: 'assistant', content: [{
          type: 'tool_use', id: 'tu-unknown', name: 'FutureTool', input: { content: 'LOW ENTROPY PRIVATE CONTENT' },
        }] },
      }),
    ].join('\n') + '\n');
    const stdin = JSON.stringify({ session_id: 'sess-unknown-tool', transcript_path: transcript, cwd: join(tmp, 'ws') });
    await runHook(['session-end'], { stdin, write: sink().write, transcriptRoot, spawnPush: () => {} });

    expect(await readSessionReceiptsTail(10)).toEqual([]);
    expect(existsSync(marker)).toBe(false);
    expect((await readHeartbeatTail(5)).some(
      (row) => row.reason === 'memorable_relay_skipped_unallowlisted',
    )).toBe(true);
  });
