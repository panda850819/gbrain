...
import {
  ...
  normForGrounding,
  maskAssistantAuthoredTranscript,
  extractQuoteSpans,
  groundQuote,
  repairBody,
...
describe('groundQuote — the repair ladder', () => {
  ...
  test("assistant-authored text cannot pass as the user's verbatim quote", () => {
    const roleAware = grounded(maskAssistantAuthoredTranscript(transcript));
    expect(groundQuote('memory systems fail at the', roleAware).status).toBe('exact');
    expect(groundQuote('mechanical checker beats an LLM judge', roleAware).status).toBe('none');
  });
