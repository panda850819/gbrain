...
describe('redactedToolCallsJson — the one artifact that leaves the machine', () => {
  ...
  test('unquoted assignments and nested structures redact too; span filter still applies', async () => {
    const out = await redactedToolCallsJson(
      [
        ...
        { name: 'shell', input: { command: ['bash', '-lc', 'api_key: Ab3xK9mQ2pR7sT1vW4yZ8bC5dE6f'] } },
  ...
  test('free-form write/edit payloads are excluded while disclosed path/outcome fields survive', async () => {
    const out = await redactedToolCallsJson(
      [
        { name: 'Write', input: { file_path: '/repo/private.txt', content: 'LOW ENTROPY PRIVATE CONTENT' }, result: { ok: true } },
        { name: 'Edit', input: { file_path: '/repo/a.ts', old_string: 'private old text', new_string: 'private new text', replace_all: false } },
      ],
      [0, 0],
      0,
    );
    expect(out).toContain('/repo/private.txt');
    expect(out).toContain('/repo/a.ts');
    expect(out).toContain('"ok":true');
    expect(out).not.toContain('LOW ENTROPY PRIVATE CONTENT');
    expect(out).not.toContain('private old text');
    expect(out).not.toContain('private new text');
  });

  test('unknown tools and fields fail closed instead of widening egress', async () => {
    await expect(redactedToolCallsJson(
      [{ name: 'FutureTool', input: { query: 'private' } }], [0], 0,
    )).rejects.toThrow('unknown tool');
    await expect(redactedToolCallsJson(
      [{ name: 'Read', input: { file_path: '/repo/a', future_content: 'private' } }], [0], 0,
