...
describe('applyTriageRescue — fires only on verified buried signal', () => {
  ...
  test("assistant-authored segments cannot rescue a transcript as the user's signal", () => {
    const assistantTranscript = [
      'user: please summarize the deployment status',
      'assistant: the retention problem is really an onboarding problem wearing a trench coat',
      'and the referral program cannibalizes organic signups at three times the cost',
    ].join('\n');
    const d = applyTriageRescue(verdict({
      segments: [
        { quote: 'the retention problem is really an onboarding problem wearing a trench coat' },
        { quote: 'the referral program cannibalizes organic signups at three times the cost' },
      ],
    }), assistantTranscript, THRESHOLD);
    expect(d).toEqual({ pass: false, rescued: false, verified_segments: 0 });
  });
