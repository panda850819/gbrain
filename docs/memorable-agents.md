...
| Harness | When | What |
|---|---|---|
| Claude Code | session end (hook) | full parsed window's allowlisted tool calls, span-aligned with the corpus; file contents and edit payloads are dropped, and an unknown tool/field stops the relay |
| Codex | session end (trust-gated `hooks.json` entry managed by `gbrain bootstrap`) | rollout calls with the same allowlisted argument policy (no per-call success flags — codex does not persist them) |
