...
- If the user is replying to a cron report with `待人工` / `invalid type` / `missing created/updated`, do not stop just because the current `gbrain frontmatter audit --json` is green. The cron may include semantic/schema checks beyond the structural audit. Reconstruct the pending set from the cron summary, inspect the named paths, and fix the reported semantic fields directly.
  - For `.raw/...` evidence / article / source-pack/schema files that are missing an opening frontmatter fence, it is safe to prepend minimal metadata instead of escalating if the filename/path/content gives the subject. Use `type: note`, infer `area: research` for research/source-pack/article/schema paths, set `created` from path date, filename date, published/captured date, or file mtime when no better signal exists, and set `updated` to today's date. Derive `title` from the first H1 when present, otherwise from the filename. Preserve source fields such as `source_url`, `author`, `source_kind`, `raw_kind`, and `topic` when obvious from the first lines or path. Create a reversible `*.fmguard-YYYY-MM-DD.bak` before writing, then validate each named file one-by-one and run the global audit again.
  - `gbrain frontmatter validate` may validate only the last positional target if passed several files together. Validate pending files in a loop, one path per command, or validate their parent directory.
