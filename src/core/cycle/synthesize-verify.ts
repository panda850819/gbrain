...
/**
 * Mask explicitly assistant-authored turns while preserving byte offsets.
 * Dream quotes are promised as the user's words; matching against the whole
 * transcript would let an assistant sentence pass as Panda's verbatim quote.
 * Unknown/unlabelled meeting transcripts remain untouched, while the common
 * `user:` / `assistant:` and minion transcript heading formats are role-aware.
 */
export function maskAssistantAuthoredTranscript(content: string): string {
  ...
  return content.replace(/[^\n]*(?:\n|$)/g, (segment) => {
    ...
    const heading = line.match(/^\s*#{1,6}\s+Message\s+\d+\s*[-—]\s*(user|assistant)\s*$/i);
    const inline = line.match(/^\s*(?:#{1,6}\s+)?(?:\*\*)?(user|assistant)(?:\s*\([^)]*\))?\s*:(?:\*\*)?/i);
    const jsonRole = line.match(/^\s*["']role["']\s*:\s*["'](user|assistant)["']/i);
    const role = heading?.[1] ?? inline?.[1] ?? jsonRole?.[1];
    if (role) activeRole = role.toLowerCase() as 'user' | 'assistant';
    if (activeRole !== 'assistant') return segment;
...
export async function verifyAndRepairDreamPages(
  ...
  for (const [rawSource, group] of byTranscript) {
    ...
    const userAuthoredContent = maskAssistantAuthoredTranscript(t.content);
    const { norm, map } = normalizeForGrounding(userAuthoredContent);
    const grounded: GroundedTranscript = { content: userAuthoredContent, norm, map };
