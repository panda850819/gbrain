...
async function runPhaseSynthesizeInner(
  ...
  try {
    ...
    for (const target of [config.reflectionsPrefix, config.originalsPrefix]) {
      if (!matchesSlugAllowList(`${target}/__dream_policy_probe__`, allowedSlugPrefixes)) {
        return failed(makeError(
          ...
          `dream write target "${target}" is not approved by dream_synthesize_paths.globs`,
...
/**
 * Validate a configured dream namespace before it reaches a subagent write
 * allow-list or SQL LIKE prefix. A namespace-root target would widen writes to
 * the whole output tree, so it fails closed just like malformed wildcard input.
 */
export function normalizeDreamNamespace(
  raw: string,
  key: string,
  outputRoot: string,
  fallback: string,
  ...
  if (SUMMARY_SLUG_RE.test(trimmed) && trimmed !== outputRoot) return trimmed;
  const reason = trimmed === outputRoot ? 'is the output root' : 'is not a valid slug prefix';
  process.stderr.write(`[dream] ${key} "${raw}" ${reason}; falling back to "${fallback}".\n`);
  return fallback;
}
...
export async function loadDreamNamespaces(
  ...
  const resolvePrefix = async (
    ...
    // dream.write_targets is the downstream canonical filing contract. Keep
    // it authoritative; upstream's lane-specific key remains the fallback for
    // brains that adopted the newer taxonomy settings.
