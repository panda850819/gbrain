...
[gbrain]  - That CLI sends an ALLOWLISTED, REDACTED subset of tool calls
[gbrain]    (commands, file paths, URLs, queries + success/failure booleans)
[gbrain]    and a <=200-char task line to Memorable's extraction API. File
[gbrain]    contents and edit payloads are excluded; an unknown tool/field
[gbrain]    stops the relay. Redaction runs first (vendor-key + high-entropy
[gbrain]    secret scan), but redaction is best-effort, not a guarantee.
 ...
 */
interface MemorableToolPolicy {
  allow: readonly string[];
  drop?: readonly string[];
}

// Consent is category-specific (commands, paths, URLs, queries, outcomes), so
// the executable policy is explicit too. Content-bearing edit/write fields are
// known and deliberately dropped; an unknown tool or field throws so a future
// harness shape cannot silently widen egress beyond the accepted disclosure.
const MEMORABLE_TOOL_POLICIES: Readonly<Record<string, MemorableToolPolicy>> = {
  Bash: { allow: ['command', 'timeout', 'run_in_background', 'dangerouslyDisableSandbox'], drop: ['description'] },
  shell: { allow: ['command', 'cwd', 'timeout_ms'] },
  Read: { allow: ['file_path', 'offset', 'limit', 'pages'] },
  Write: { allow: ['file_path'], drop: ['content'] },
  Edit: { allow: ['file_path', 'replace_all'], drop: ['old_string', 'new_string'] },
  MultiEdit: { allow: ['file_path'], drop: ['edits'] },
  Glob: { allow: ['pattern', 'path'] },
  Grep: { allow: ['pattern', 'path', 'glob', 'output_mode', '-B', '-A', '-C', '-n', '-i', 'type', 'head_limit', 'offset', 'multiline'] },
  WebFetch: { allow: ['url', 'prompt'] },
  WebSearch: { allow: ['query', 'allowed_domains', 'blocked_domains'] },
  search_brain: { allow: ['query'] },
};

export class MemorableToolPolicyError extends Error {
  readonly code = 'MEMORABLE_TOOL_POLICY';
}

function allowlistedMemorableToolCall(call: ToolCallRecord): ToolCallRecord {
  const policy = MEMORABLE_TOOL_POLICIES[call.name];
  if (!policy) throw new MemorableToolPolicyError(`Memorable relay refused unknown tool: ${call.name}`);
  if (call.input === null || call.input === undefined) return { name: call.name, input: null, ...(call.result && { result: call.result }) };
  if (typeof call.input !== 'object' || Array.isArray(call.input)) {
    throw new MemorableToolPolicyError(`Memorable relay refused non-object input for tool: ${call.name}`);
  }
  const safe: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(call.input as Record<string, unknown>)) {
    if (policy.drop?.includes(key)) continue;
    if (!policy.allow.includes(key)) {
      throw new MemorableToolPolicyError(`Memorable relay refused unknown field ${call.name}.${key}`);
    }
    safe[key] = value;
  }
  return { name: call.name, input: safe, ...(call.result && { result: call.result }) };
}
...
export async function redactedToolCallsJson(calls: ToolCallRecord[], turnIndexes: number[], startTurnIndex: number): Promise<string> {
  ...
  const span = calls
    .filter((_c, i) => (turnIndexes[i] ?? 0) >= startTurnIndex)
    .map(allowlistedMemorableToolCall);
