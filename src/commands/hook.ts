...
import { MemorableToolPolicyError, memorableGateAllowed, recordAndRelayReceipt, redactedToolCallsJson } from '../core/context/hook-heartbeat.ts';
    ...
    if (!conf.ok) {
      ...
      if (bytesN > 0 && turnsN === 0) {
        ...
        if (segmentMode !== 'skip_covered') {
          ...
          let toolCallsJson = '[]', toolPolicyRefused = false;
          try {
            ...
            // No proof bit means no receipt/relay, even if corpus scanning
            // completed before tool-policy validation refused the shape.
            redactionsN = undefined;
            toolPolicyRefused = err instanceof MemorableToolPolicyError;
            if (!toolPolicyRefused) degrade('scan_unavailable');
          }
          ...
          if (memorableAllowed && discoveryWasGuess) {
            ...
            deferredReasons.push(toolPolicyRefused
              ? 'memorable_relay_skipped_unallowlisted'
              : 'memorable_relay_skipped_unscanned');
